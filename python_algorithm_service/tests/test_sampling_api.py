...
def test_batch_sampling_for_high_risk_batch_expands_ratio() -> None:
    ...
    assert body["result"]["sample_count"] == 2
    assert body["result"]["selected_task_ids"] == ["t-1", "t-2"]
...
def test_batch_sampling_rejects_unknown_task_risk_level_with_error_envelope() -> None:
    client = TestClient(create_app())
    ...
    response = client.post(
        ...
        json={
            "batch_id": "batch-1",
            "project_id": "project-1",
            "task_pool": [{"task_id": "t-1", "risk_level": "urgent"}],
